...
function createLink(elementView) {
  ...
  if (startingElem != currentElement.id && !linked) {
    ...
    if (inclLabel) { finalLink.appendLabel({ position: 0.8, attrs: { text: { text: "N" } } }); }
    ...
    if (connectionBetween(startingElement, currentElement, 'tm.Weak_Entity', 'tm.Identifying_Relationship')) {    
      finalLink.attr('line/strokeWidth', 5);
      finalLink.label(0, { attrs: { text: { text: '1' } } });
    }
    finalLink.addTo(graph);

    $('#yesButton').click(() => {
      ($('#query_modal_text').html().includes('aggregation')) ? 
        finalLink.attr('line/stroke', 'red') : 
        finalLink.appendLabel({ position: 0.8, attrs: { text: { text: "Superclass" } } });;
      addState();
    })
...
function connectionAllowed(starting, current) {
  ...
  if (!connectionAllowed) {
    warningAlert(`Connection between ${starting.prop('type').substring(3).toUpperCase().bold()} and 
...
paper.on("link:mouseenter", function (linkView) {
  ...
  var infoTool = new joint.linkTools.Button({
    ...
    action: function (evt) {
      ...
      //   attrs: { text: { text: window.prompt("Cardinality: ") } },
      // });
      showHelpbox(linkView.model);
    },
  });
...
paper.on("blank:pointerdown", function (evt, x, y) {
  var helpbox_area = document.getElementById("help");
  if (helpbox_area.style.display === "block") {
    var mainCanvas = document.getElementById("main-canvas");
    mainCanvas.classList.remove("col-sm-7");
    mainCanvas.classList.add("col-sm-9");
    document.getElementById('element-helpbox-content').style.display = "none";
    document.getElementById('link-helpbox-content').style.display = "none";
    helpbox_area.style.display = "none";
  }
